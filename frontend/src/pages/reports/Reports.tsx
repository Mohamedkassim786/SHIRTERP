...
export default function Reports() {
  ...
  const tabs: { key: ReportType; labelKey: string, defaultLabel: string }[] = [
    { key: 'pl',          labelKey: 'reports.tabs.pl', defaultLabel: '📊 P&L Report' },
    { key: 'sales',       labelKey: 'reports.tabs.sales', defaultLabel: '🛒 Sales Report' },
    { key: 'purchase',    labelKey: 'reports.tabs.purchase', defaultLabel: '📦 Purchase Report' },
    { key: 'stock',       labelKey: 'reports.tabs.stock', defaultLabel: '📋 Stock Report' },
    { key: 'gst',         labelKey: 'reports.tabs.gst', defaultLabel: '🧾 GST Report' },
    { key: 'outstanding', labelKey: 'reports.tabs.outstanding', defaultLabel: '💰 Outstanding' },
    { key: 'customer',    labelKey: 'reports.tabs.customer', defaultLabel: '👥 Customer Report' },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" onClick={handlePrint}><Printer className="h-4 w-4 mr-2" />{t('reports.print', 'Print / Export')}</Button>
      </div>
      ...
      <div className="flex flex-wrap gap-2 border-b pb-2">
        {tabs.map(tItem => (
          <button key={tItem.key} onClick={() => setReportType(tItem.key)}
            className={`px-3 py-1.5 rounded-lg font-medium text-sm ${reportType === tItem.key ? 'bg-primary text-slate-900' : 'bg-white text-slate-400 hover:bg-slate-200'}`}>
      ...
      <div className="flex flex-wrap gap-3 items-end p-4 bg-slate-50/50 rounded-lg">
        <div><Label className="text-xs">{t('reports.fromDate', 'From Date')}</Label><AnimatedInput type="date" className="w-36" value={dateFrom} onChange={e => setDateFrom(e.target.value)} /></div>
        <div><Label className="text-xs">{t('reports.toDate', 'To Date')}</Label><AnimatedInput type="date" className="w-36" value={dateTo} onChange={e => setDateTo(e.target.value)} /></div>
      ...
      {!isLoading && reportType === 'sales' && data && (
        <div className="space-y-4">
          <Card><CardContent className="p-4 flex justify-between items-center"><span className="font-medium">{t('reports.sales.total', 'Total Sales (Period)')}</span><span className="text-xl font-bold text-green-600">₹{data.total?.toLocaleString('en-IN')}</span></CardContent></Card>
          <DataTable columns={[
            { key: 'invoiceNumber', label: t('reports.sales.cols.invoice', 'Invoice #') },
            { key: 'date', label: t('reports.sales.cols.date', 'Date'), render: (r: any) => new Date(r.date).toLocaleDateString('en-IN') },
            { key: 'customer', label: t('reports.sales.cols.customer', 'Customer'), render: (r: any) => r.customer?.name },
            { key: 'subTotal', label: t('reports.sales.cols.subTotal', 'Sub Total'), render: (r: any) => `₹${r.subTotal?.toLocaleString('en-IN')}` },
            { key: 'gstAmount', label: t('reports.sales.cols.gst', 'GST'), render: (r: any) => `₹${r.gstAmount?.toLocaleString('en-IN')}` },
            { key: 'totalAmount', label: t('reports.sales.cols.total', 'Total'), render: (r: any) => <span className="font-bold">₹{r.totalAmount?.toLocaleString('en-IN')}</span> },
      ...
      {!isLoading && reportType === 'purchase' && data && (
        <div className="space-y-4">
          <Card><CardContent className="p-4 flex justify-between items-center"><span className="font-medium">{t('reports.purchase.total', 'Total Purchases (Period)')}</span><span className="text-xl font-bold text-orange-600">₹{data.total?.toLocaleString('en-IN')}</span></CardContent></Card>
          <DataTable columns={[
            { key: 'poNumber', label: t('reports.purchase.cols.po', 'PO #') },
            { key: 'createdAt', label: t('reports.purchase.cols.date', 'Date'), render: (r: any) => new Date(r.createdAt).toLocaleDateString('en-IN') },
            ...
            { key: 'totalAmount', label: t('reports.purchase.cols.total', 'Total'), render: (r: any) => <span className="font-bold">₹{r.totalAmount?.toLocaleString('en-IN')}</span> },
      ...
      {!isLoading && reportType === 'gst' && data && (
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Card className="border-l-4 border-l-green-500"><CardHeader className="pb-1"><CardTitle className="text-xs text-slate-400">{t('reports.gst.output', 'Output GST (Collected)')}</CardTitle></CardHeader><CardContent><div className="text-xl font-bold text-green-600">₹{data.outputGst?.toFixed(2)}</div></CardContent></Card>
            <Card className="border-l-4 border-l-orange-500"><CardHeader className="pb-1"><CardTitle className="text-xs text-slate-400">{t('reports.gst.input', 'Input GST (Paid on Purchases)')}</CardTitle></CardHeader><CardContent><div className="text-xl font-bold text-orange-600">₹{data.inputGst?.toFixed(2)}</div></CardContent></Card>
            <Card className="border-l-4 border-l-primary"><CardHeader className="pb-1"><CardTitle className="text-xs text-slate-400">{t('reports.gst.net', 'Net GST Payable')}</CardTitle></CardHeader><CardContent><div className="text-xl font-bold text-primary">₹{data.netGst?.toFixed(2)}</div></CardContent></Card>
          </div>
          <DataTable columns={[
            { key: 'invoiceNumber', label: t('reports.gst.cols.invoice', 'Invoice #') },
            { key: 'date', label: t('reports.gst.cols.date', 'Date'), render: (r: any) => new Date(r.date).toLocaleDateString('en-IN') },
            { key: 'customer', label: t('reports.gst.cols.customer', 'Customer'), render: (r: any) => r.customer?.name },
            { key: 'subTotal', label: t('reports.gst.cols.taxable', 'Taxable Value'), render: (r: any) => `₹${r.subTotal?.toFixed(2)}` },
            { key: 'gstAmount', label: t('reports.gst.cols.gst', 'GST Amount'), render: (r: any) => `₹${r.gstAmount?.toFixed(2)}` },
            { key: 'totalAmount', label: t('reports.gst.cols.total', 'Invoice Total'), render: (r: any) => `₹${r.totalAmount?.toFixed(2)}` },
          ]} data={data.invoices || []} searchKey="invoiceNumber" />
        </div>
      )}
      ...
      {!isLoading && reportType === 'outstanding' && data && (
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <Card className="border-l-4 border-l-red-500"><CardHeader className="pb-1"><CardTitle className="text-xs text-slate-400">{t('reports.out.customerOutstanding', 'Customer Outstanding')}</CardTitle></CardHeader><CardContent><div className="text-xl font-bold text-red-600">₹{data.totalCustomerOutstanding?.toLocaleString('en-IN')}</div></CardContent></Card>
            <Card className="border-l-4 border-l-orange-500"><CardHeader className="pb-1"><CardTitle className="text-xs text-slate-400">{t('reports.out.vendorOutstanding', 'Vendor Outstanding')}</CardTitle></CardHeader><CardContent><div className="text-xl font-bold text-orange-600">₹{data.totalSupplierOutstanding?.toLocaleString('en-IN')}</div></CardContent></Card>
          </div>
          <div>
            ...
            <DataTable columns={[
              ...
              { key: 'outstandingBalance', label: t('reports.out.cols.outstanding', 'Outstanding'), render: (r: any) => <span className="font-bold text-red-600">₹{r.outstandingBalance?.toLocaleString('en-IN')}</span> },
          ...
          <div>
            ...
            <DataTable columns={[
              ...
              { key: 'outstandingBalance', label: t('reports.out.cols.outstanding', 'Outstanding'), render: (r: any) => <span className="font-bold text-orange-600">₹{r.outstandingBalance?.toLocaleString('en-IN')}</span> },
      ...
      {!isLoading && reportType === 'customer' && data && (
        <DataTable columns={[
          ...
          { key: 'totalRevenue', label: t('reports.customer.cols.revenue', 'Total Revenue'), render: (r: any) => <span className="font-bold text-green-600">₹{r.totalRevenue?.toLocaleString('en-IN')}</span> },
          { key: 'outstandingBalance', label: t('reports.customer.cols.outstanding', 'Outstanding'), render: (r: any) => <span className={r.outstandingBalance > 0 ? 'text-red-600' : 'text-green-600'}>₹{r.outstandingBalance?.toLocaleString('en-IN')}</span> },
