...
import {
  ...
  Users, Truck, Settings, BarChart3, IndianRupee, Receipt, Factory, UserCheck, Landmark, X
} from 'lucide-react';
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      `}>
        ...
        <nav className="flex-1 overflow-y-auto py-5 space-y-6">
          {navGroups.map((group) => (
            <div key={group.label}>
              <p className="px-7 text-[11px] font-bold text-slate-400 uppercase tracking-[0.14em] mb-2">{group.label}</p>
        ...
        <div className="p-6 border-t border-slate-100 flex-shrink-0">
          <div className="flex items-center gap-4 mb-4 px-2">
            <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center text-white font-bold text-lg flex-shrink-0 shadow-md shadow-blue-200/60">
              {user?.name?.charAt(0).toUpperCase() || 'A'}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-[16px] font-bold text-slate-900 truncate leading-tight">{user?.name}</p>
              <p className="text-[13px] text-slate-400 truncate capitalize font-medium">{user?.role}</p>
            </div>
          </div>
          <Button
            variant="ghost"
